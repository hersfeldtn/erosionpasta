...
;For this to work properly, there needs to be an existing selection mask called "noise.png" (doesn't matter what specifically is on it) in the directory.
	...
	;Message when completing any step
	endmes(opt)
	{
		MsgBox
		(
	...
	;functions for different parts of the process
	prelim(start:=false)
	{
		startup()
		;produce noise map
	...
	;Second erosion cycle (mountain-focused)
	pass2(start:=false)
	{
		if (start)
		{
	...
	pass4(start:=false)
	{
		...
		;I recommend applying a final exponential filter with exponents as 2, but left it out here.
