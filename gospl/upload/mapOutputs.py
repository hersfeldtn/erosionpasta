...
class mapOutputs:
    ...
    def buildLonLatMesh(self, res=0.1, nghb=3, box=None):
        ...
        if self.nx is None:
            ...
            if box is None:
                ...
                self.lon = np.linspace(-180.0+res/2, 180.0-res/2, self.nx)
                self.lat = np.linspace(-90.0+res/2, 90.0-res/2, self.ny)
            else:
                self.nx = int((box[2] - box[0]) / res)
                self.ny = int((box[3] - box[1]) / res)
                self.lon = np.linspace(box[0]+res/2, box[2]-res/2, self.nx)
                self.lat = np.linspace(box[1]+res/2, box[3]-res/2, self.ny)
    ...
    def buildLonLatMesh_e(self, res=0.1, nghb=3, box=None):
    ...
    def exportNetCDF_e(self, ncfile):
